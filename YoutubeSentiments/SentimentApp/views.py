...
mse_error = calculateMetrics("CNN", predict, y_test)#call function to plot LSTM crop yield prediction
...
def TrainCNN(request):
    if request.method == 'GET':
        ...
        output+='<td><font size="" color="black">CNN Algorithm</td><td><font size="" color="black">'+str(mse_error)+'</td></tr>'
...
def SignupAction(request):
    if request.method == 'POST':
        ...
        status = 'none'
        con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '12345', database = 'sentimentapp',charset='utf8')
        with con:
            cur = con.cursor()
            ...
            for row in rows:
                if row[0] == email:
                    ...
                    break
        if status == 'none':
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = '12345', database = 'sentimentapp',charset='utf8')
